...
The database contains 29 entries: 17 measurement methods outside the distance ladder and 12 components of the distance ladder. The distance ladder is not treated as a single measurement method, because it does not correspond to a fixed observational procedure. Instead, it is an inference chain in which the indicators used at individual rungs can be replaced by alternative ones. Each rung is calibrated using overlapping objects with the previous rung, allowing the absolute distance scale to be propagated to increasingly distant galaxies. The 12 distance ladder components comprise 2 geometric anchors used in the distance ladder, 5 first-rung indicators and 5 second-rung indicators. This allows their different physical principles and observational ranges to be compared directly.
...
The interactive visualisation and data-processing scripts were developed with assistance from AI tools. The author designed the database, made method-classification decision, curated the literature and reviewed the final outputs.

### Data and Code Availability

The interactive explorer is available at https://zhnicex.github.io/hubble-constant-measurement-methods/.

The source database, machine-readable data, conversion script, and explorer source code are available in the project repository at https://github.com/zhnicex/hubble-constant-measurement-methods.
